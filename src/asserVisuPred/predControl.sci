...
function [epsilon_out, cmMc] = epsilonX(s_in,sm_in,L_in, Te_in)
  epsilon_out = pinv(L_in)*(s_in-sm_in);
  epsilon_out = (abs(epsilon_out)>1E-8).*epsilon_out;
	cmMc= homogeneousMatrixFromPos(epsilon_out);
...
//-----------------------------------------------//
// Compute sd on horizon
// date 6/12/2011
//-----------------------------------------------//
function sd_out = constSdOnHor(sd_in,Np_in)
//%
// This function duplicates the sd_in desired features on the horizon
// then sd is constant on horizon 
	sd_out =[];
  for i=1:Np_in 
   sd_out = [sd_out ; sd_in];
  end
...
// date 04/01/2010
//-----------------------------------------------//
function cost = ga_corrCostHorizon(Np,sm,Q,e0,sStar)
  //%
  // compute the cost function on a whole horizon with correction
