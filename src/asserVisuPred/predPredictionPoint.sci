...
// date 1/06/2011
//-----------------------------------------------//
function [sm_out,Z_out] = ga_predGlobalPoint(s_in,Z_in,v_in,Te_in)
  //% 
  // update the predicted model  
  // 
  // @param Z_in is an arbitrarily chosen depth
  // @param v_in is the camera velocity or the control at time t
  // @param Te_in is  the sampling time
...
// date 01/06/2011
//-----------------------------------------------//
function sm = ga_predHorGlobal2dPoint(sm0,U,Te,Z)
  //
  // predict sm given the whole control U on the horizon
  //
  // U is the controlover the time horizon 
  // sm0 is the fist model
  // Te is the sampling time
  // Z is the depth it is fixed on the horizon
  ...
  for i=1:N
   // compute the next local prediction for 2d features
    smNext = ga_predGlobal2dPoint(smPrec,U((i-1)*6+1:(i-1)*6+6),Te,Z);
    // store the prediction
    sm = [sm ; smNext];
    // update smPrecedent
    smPrec = smNext;     
