...
// 
//-----------------------------------------------//
function sm_out = predHorLoc2dMireConst(s_in,Z_in,U_in,Te_in,Np_in)
   L_out  = computeL_global(s_in,Z_in);
   smPrec = s_in;
   sm_out = [];
   ndof      = length(U_in)/Np_in; // number degree of freedom   
   for i=1:Np_in
      v_in =U_in((i-1)*ndof+1:(i-1)*ndof+ndof);
      smNext = smPrec+ Te_in*L_out*v_in;
      sm_out = [sm_out ; smNext];     
      smPrec = smNext; 
...
//-----------------------------------------------//
// Prediction model from position estimation
//
// date 4/08/2011
//
//-----------------------------------------------//

function [sm_out, cMcN_out, Z_out] = preHorGlobalMirePos(oP_in,cMo_in,U_in,Te_in,Np_in)
  
cMcN_out = ga_predHorPosition(U_in,Te_in,Np_in);
sm_out =[];
Z_out  =[];
oMc_here=inv(cMo_in);
for i=1:Np_in
    cMcN         = cMcN_out(((i-1)*4+1:(i-1)*4+4),:)  ;         // resulting motion
    oMcN         = oMc_here*cMcN;
    cNP          = changeFrameMire(oP_in,inv(oMcN));
    Z_out        = [Z_out ; cNP(1:3:$)];
    sN           = projectMireDirect(cNP);
    sm_out       = [sm_out ; sN];
end
...
//------------------------------------------------//
//
// Linear correction of the estimated feature
//
// date 08/08/2011
// author dune claire
//
// brief : given a trajectory of features and measure 
// features, compensate for the error with a constant correction
//------------------------------------------------//

function sm_out = linearCorrection(s_in,eps_in, Np_in)
  
nbpts  = length(s_in)/(Np_in) ;
A      = eye(nbpts,nbpts);
B      = [];
for i=1:Np_in
    B  = [B;A]; 
...
// author dune claire
//
// same as ga_predHorGlobalMire but 
// uses the first sm and Z to compute cP and then
// compute all camera position/the first
//------------------------------------------------//
function sm_out = cld_predHorGlobalMire(s_in,Z_in,U_in,Te_in,Np_in)
  //% 
  // update the predicted model using the trajectory of the camera
  // we do not know cMo but we can compute an approximation of cP using s_in and Z_in
	
	// compute the number of points contained in s
  N_points = length(s_in)/2; //number of tracked points
  if(length(Z_in)~=N_points)    //build  Z vector if necessary
     Z_current = Z_in(1)*ones(N_points,1);
  else
     Z_current = Z_in;
  end
	
  c0P =[];     //to store the coeff
  for (i=1:N_points)
     c0P       =[c0P
                 s_in((i-1)*2+1)*Z_current(i)
                 s_in((i-1)*2+2)*Z_current(i)
	...
	c0McN_out = ga_predHorPosition(U_in,Te_in,Np_in);
	sm_out =[];
	Z_out  =[];
	
	for i=1:Np_in
    cMcN         = c0McN_out(((i-1)*4+1:(i-1)*4+4),:)  ;         // resulting motion
		cNP          = changeFrameMire(c0P,inv(cMcN));
		Z_out        = [Z_out ; cNP(1:3:$)];
    sN           = projectMireDirect(cNP);
		sm_out       = [sm_out ; sN];
  end
