...
//---------------------------------------//
// compute Lz matrix for one point
// author : Claire Dune
// date : septembre 2013
//---------------------------------------//
function L = matLzPoint6ddl(x,y)
// compute the Lz Matrix described in Folio & Cadenat
  L = [0 , 0,  -1, -y ,-x,  0] ;
endfunction
...
//---------------------------------------//
// compute the Lz matrix for n 2D points
// to estimate z
//
// only the projection of the points in the
// image plane are known
//
// author : Claire Dune
// date :  september 2013 (ICRA)
//---------------------------------------//
function L = matLzMire6ddl(p)
  N = length(p)/2;
  L=[];
  for i=1:N
      L= [L; matLzPoint6ddl(p((i-1)*2+1),p((i-1)*2+2))];
  end
endfunction

//---------------------------------------//
// Compute the ODE relative to the visual
// servoing control law, see Folio's
//
//
// author : Claire Dune
// date :  september 2013 (ICRA)
//---------------------------------------//
function Xdot =vsOde(t,X,v)
  // t mandatory
  // L the interaction matrix
  // v the velocity
  N_in    = length(X)/3;
  s_in    =(X(1:2*N_in));
  Z_in    =(X(2*N_in+1:$)); 
  
  L_in    = matIntMireC(s_in,Z_in);
  Lz_in   = matLzMire6ddl(s_in);
