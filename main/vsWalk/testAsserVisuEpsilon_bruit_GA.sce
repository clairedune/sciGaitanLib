...
// author Claire Dune
// date 10/04/2011
// brief : add a noise on the mesured s
//  correct by removing the real s
...
//        Problem Setting
//---------------------------------------------------------------------//
posecdesMo   = [0 0 1 0 0 %pi/12];             // desired pose target/object 
posecinitMo  = [1 1 2 0 -%pi/2 -%pi/12];             // init pose target/object   
...
while( iter < iterMax)
  ...
  // noise on s
  sreal      = s ;
  smeasure   = s + ((rand(s,'normal')-1/2)/500);
  smodel     = smeasure + Te* L*vcam';
  
  Z          = cP(3:3:$);                                  // compute Z
  
  // compute error
  epsilon    = smodel-smeasure;
  scorrection= smodel-epsilon;
  e          = scorrection-sDes;
  norme      = [norme;norm(e)];  
  
  // compute the interaction matrix
  L          = Lfunction(smodel,Z);                             
 ...
 //save data for display
  ...
  Scorrection= [Scorrection scorrection];
  ...
  if (iter>2)
      for i=1:Nbpts
        ...
        plot(Smodel((i-1)*2+1,:),Smodel((i-1)*2+2,:),'b')
        plot(Smeasure((i-1)*2+1,:),Smeasure((i-1)*2+2,:),'g')
        plot(Scorrection((i-1)*2+1,:),Scorrection((i-1)*2+2,:),'c')
      end  
  end
  show_pixmap();
