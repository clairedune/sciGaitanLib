...
poserFinal     = [1 0.5 h 0 0 -0.1]; // CoM final pose in the world reference frame
...
while( iter < maxiter & STOP_CRITERION==%F)
 ...
 // vcam   = inv(rVc)*vrobot;
  ...
  if(OPT_DISTURB&iter==DISTURB_ITER)
    vrobotReal = vrobotReal + disturb';
  ...
  VRobotReal = [VRobotReal;RobotReal([2;5;8])'];
